...
type Summary = { set_count:number; unique_sets:number; piece_count:number; loose_piece_count:number; minifig_count:number; minifig_value:string };
type OwnedSet = { id:number; quantity:number; contributed_piece_count:number; set:{ id:number; set_num:string; name:string; image_url?:string; official_piece_count:number; theme_name:string } };
...
function AddPanel({ tab, onAdded }:{ tab:Exclude<Tab,"overview">; onAdded:()=>void }) {
  ...
  async function search() {
    setSearching(true);
    try {
      const res = await api.get<LibraryPickerResult[]>(ENDPOINTS.libraryPickerLookup,{params:{q,type:mode,limit:12}});
      setResults(res.data);
  ...
  return <div className="collection-add">
    ...
    <form onSubmit={(e)=>{e.preventDefault();search();}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder={tab === "sets" ? "Search set name or number…" : tab === "parts" ? "Search piece, color or ID…" : "Search minifigure or theme…"}/><button>{searching?"Searching…":"Search"}</button></form>
    {results.length>0 && <div className="collection-search-results">{results.map(item=><button key={`${item.type}-${item.id}`} onClick={()=>add(item)}>
      <span>{item.image_url?<img src={item.image_url} alt=""/>:"◇"}</span><div><strong>{item.title}</strong><small>{item.subtitle}</small></div><b>+ Add</b>
    </button>)}</div>}
  </div>;
}
    ...
    <section className="collection-wrap collection-content">
      ...
      {tab==="overview" && <div className="collection-overview">
        <div className="collection-stats">
          ...
          <article className="green"><span>FIGURE VALUE</span><strong>{money(summary?.minifig_value||0)}</strong><small>Current catalog value</small></article>
        </div>
        <div className="collection-welcome">
          <div><p className="collection-eyebrow">YOUR COLLECTION, WORKING SMARTER</p><h2>Every item you add unlocks more possibilities.</h2><p>Add a complete set and all of its pieces immediately count toward other builds. Add loose finds one by one. We’ll keep the math out of your way.</p><button onClick={()=>setTab("sets")}>Add your first set →</button></div>
      ...
      {tab==="sets" && <><AddPanel tab="sets" onAdded={load}/><div className="collection-section-head"><div><p>COMPLETE SETS</p><h2>My build library</h2></div><span>{sets.length} unique sets</span></div><div className="owned-set-grid">{sets.map(row=><article key={row.id}><div>{row.set.image_url?<img src={row.set.image_url} alt=""/>:"◇"}<b>×{row.quantity}</b></div><small>{row.set.theme_name} · {row.set.set_num}</small><h3>{row.set.name}</h3><p><strong>{row.contributed_piece_count.toLocaleString()}</strong> pieces added to your inventory</p><button onClick={()=>remove(ENDPOINTS.collectionSets,row.id)}>Remove</button></article>)}</div>{!sets.length&&<p className="empty-copy">No sets yet. Search above to add your first complete build.</p>}</>}

      {tab==="parts" && <><AddPanel tab="parts" onAdded={load}/><div className="collection-section-head"><div><p>LOOSE PIECES</p><h2>Individual inventory</h2></div><span>{summary?.loose_piece_count||0} pieces</span></div><div className="owned-parts-list">{parts.map(row=><article key={row.id}><div>{row.part_color.image_url?<img src={row.part_color.image_url} alt=""/>:"◇"}</div><p><strong>{row.part_color.name}</strong><small>{row.part_color.part_id} · {row.part_color.color_name}</small></p><b>×{row.quantity}</b><button onClick={()=>remove(ENDPOINTS.collectionParts,row.id)}>Remove</button></article>)}</div>{!parts.length&&<p className="empty-copy">No loose pieces yet. Add individual finds with the search above.</p>}</>}

      {tab==="minifigs" && <><AddPanel tab="minifigs" onAdded={load}/><div className="collection-section-head"><div><p>THE MINIFIGURE VAULT</p><h2>Your collectible gallery</h2></div><span>{money(summary?.minifig_value||0)} total value</span></div><div className="fig-vault">{figs.map(row=><article className={`fig-card ${row.minifig.rarity}`} key={row.id}><div className="fig-shine"/><span className="rarity">{row.minifig.rarity}</span><div className="fig-image">{row.minifig.image_url?<img src={row.minifig.image_url} alt=""/>:"♟"}</div><small>{row.minifig.theme_name} · {row.minifig.bricklink_id}</small><h3>{row.minifig.name}</h3><div className="fig-value"><span>EST. VALUE</span><strong>{money(row.minifig.market_value)}</strong></div><button onClick={()=>remove(ENDPOINTS.collectionMinifigs,row.id)}>Remove from vault</button></article>)}</div>{!figs.length&&<p className="empty-copy">Your vault is empty. Find a minifigure above and start collecting.</p>}</>}
    </section>
  </main>;
}
