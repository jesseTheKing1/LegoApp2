    ...
    <section className="collection-wrap collection-content">
      ...
      {tab==="overview" && <div className="collection-overview">
        <div className="collection-stats">
          ...
          <article className="green"><span>TOTAL BRICKLINK VALUE</span><strong>{money(summary?.total_value||0)}</strong><small>Sets, loose pieces & figures</small></article>
        </div>
      ...
      {tab==="sets" && <><AddPanel tab="sets" onAdded={load}/><div className="collection-section-head"><div><p>COMPLETE SETS</p><h2>My build library</h2></div><span>{money(summary?.set_value||0)} BrickLink value</span></div><div className="owned-set-grid">{sets.map(row=><article key={row.id}><div>{row.set.image_url?<img src={row.set.image_url} alt=""/>:"◇"}<b>×{row.quantity}</b></div><small>{row.set.theme_name} · {row.set.year_released||"Year unknown"} · {row.set.set_num}</small><h3>{row.set.name}</h3><p><strong>{row.contributed_piece_count.toLocaleString()}</strong> pieces · BrickLink value <strong>{money(row.set.bricklink_value)}</strong></p><button onClick={()=>remove(ENDPOINTS.collectionSets,row.id)}>Remove</button></article>)}</div>{!sets.length&&<p className="empty-copy">No sets yet. Browse by theme or year above to find your first build.</p>}</>}

      {tab==="parts" && <><AddPanel tab="parts" onAdded={load}/><div className="collection-section-head"><div><p>LOOSE PIECES</p><h2>Individual inventory</h2></div><span>{summary?.loose_piece_count||0} pieces · {money(summary?.loose_parts_value||0)} BrickLink value</span></div><div className="owned-parts-list">{parts.map(row=><article key={row.id}><div>{row.part_color.image_url?<img src={row.part_color.image_url} alt=""/>:"◇"}</div><p><strong>{row.part_color.name}</strong><small>{row.part_color.part_id} · {row.part_color.color_name}</small></p><b>×{row.quantity}</b><button onClick={()=>remove(ENDPOINTS.collectionParts,row.id)}>Remove</button></article>)}</div>{!parts.length&&<p className="empty-copy">No loose pieces yet. Add individual finds with the search above.</p>}</>}

      {tab==="minifigs" && <><AddPanel tab="minifigs" onAdded={load}/><div className="collection-section-head"><div><p>THE MINIFIGURE VAULT</p><h2>Your collectible gallery</h2></div><span>{money(summary?.minifig_value||0)} BrickLink value</span></div><div className="fig-vault">{figs.map(row=><article className={`fig-card ${row.minifig.rarity}`} key={row.id}><div className="fig-shine"/><span className="rarity">{row.minifig.rarity}</span><div className="fig-image">{row.minifig.image_url?<img src={row.minifig.image_url} alt=""/>:"♟"}</div><small>{row.minifig.theme_name} · {row.minifig.bricklink_id}</small><h3>{row.minifig.name}</h3><div className="fig-value"><span>BRICKLINK VALUE</span><strong>{money(row.minifig.market_value)}</strong></div><button onClick={()=>remove(ENDPOINTS.collectionMinifigs,row.id)}>Remove from vault</button></article>)}</div>{!figs.length&&<p className="empty-copy">Your vault is empty. Find a minifigure above and start collecting.</p>}</>}
    </section>
  </main>;
}
