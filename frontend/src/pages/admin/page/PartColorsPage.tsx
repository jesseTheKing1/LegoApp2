...
function getListData<T = any>(resData: any): T[] {
  ...
  if (Array.isArray((resData as any).results)) return (resData as any).results;
  return [];
}
...
function MiniThumb({ src }: { src?: string | null }) {
  return (
    ...
    >
      {src ? <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 9, color: "#cbd5e1" }}>•</span>}
    </div>
  );
}
function PillButton({
  ...
  return (
    <button onClick={disabled ? undefined : onClick} style={styles[tone]}>
      {children}
    </button>
  );
}
...
function PartColorDetailDrawer({
  ...
  const swatches = useMemo(() => {
    ...
    for (const row of siblings) {
      ...
      const cHex = safeHex((c as any).hex) ?? safeHex(colors.find((x) => x.id === c.id)?.hex ?? null);
      ...
      {!selected ? (
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "grid", gap: 12, gridTemplateColumns: "minmax(240px, 360px) 1fr" }}>
            ...
            >
              {heroSrc ? (
                <img src={heroSrc} alt="" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
              ) : (
                <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 800 }}>No image</div>
              )}
            </div>

            <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 14, background: "white", display: "grid", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 950, color: "#111827" }}>
                    {selected.color?.name ?? "—"}
                    {selected.variant ? <span style={{ color: "#6b7280", fontWeight: 900 }}> • {selected.variant}</span> : null}
                  </div>
                  <div style={{ fontSize: 12, color: "#6b7280" }}>
                    <span style={{ fontWeight: 900, color: "#111827" }}>{selected.part?.part_id}</span> — {selected.part?.name}
                  </div>
                </div>
              </div>

              <div style={{ border: "1px solid #f1f5f9", borderRadius: 12, padding: 10, background: "#fafafa" }}>
                <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 900 }}>Your PartColor ID</div>
                <div style={{ fontSize: 13, fontWeight: 950, color: "#111827" }}>{selected.part_color_code ?? "—"}</div>
              </div>

              {(selected.image_url_1 || selected.image_url_2) && (
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {selected.image_url_1 ? (
                    <a href={selected.image_url_1} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#2563eb", fontWeight: 900 }}>
                      Open image 1
                    </a>
                  ) : null}
                  {selected.image_url_2 ? (
                    <a href={selected.image_url_2} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#2563eb", fontWeight: 900 }}>
                      Open image 2
                    </a>
                  ) : null}
                </div>
          ...
          {swatches.length > 0 && (
            <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 10, background: "white" }}>
              <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 950, marginBottom: 8 }}>Switch color</div>
                ...
                {swatches.map((s) => (
                  <button
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      gap: 8,
                      border: selected.color?.id === s.colorId ? "1px solid #111827" : "1px solid #e5e7eb",
                      background: selected.color?.id === s.colorId ? "#111827" : "white",
                      color: selected.color?.id === s.colorId ? "white" : "#111827",
                      borderRadius: 999,
                      padding: "6px 10px",
                      cursor: "pointer",
                      fontWeight: 850,
                  ...
                  >
                    <span style={{ width: 12, height: 12, borderRadius: 5, background: s.hex ?? "#e5e7eb", border: "1px solid rgba(0,0,0,.12)" }} />
                    <span style={{ whiteSpace: "nowrap" }}>{s.name}</span>
                    {s.count > 1 ? <span style={{ fontSize: 11, opacity: 0.8, marginLeft: 2 }}>+{s.count - 1}</span> : null}
                  </button>
                ))}
              </div>
            </div>
...
export default function PartColorsPage() {
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      ...
      {err && (
        <div style={{ color: "crimson", border: "1px solid rgba(220,38,38,.25)", background: "rgba(220,38,38,.06)", borderRadius: 10, padding: "8px 10px", fontSize: 12 }}>
          {err}
        </div>
      )}
      ...
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden", background: "white" }}>
        {grouped.length === 0 ? (
          ...
          grouped.map((g, idx) => {
            ...
            const thumbs = g.rows.map((r) => r.thumb_url || r.image_url_1 || r.image_url_2 || null).filter(Boolean).slice(0, 4);
            ...
            return (
              <div key={g.part.id} style={{ borderTop: idx === 0 ? "none" : "1px solid #f1f5f9" }}>
                ...
                >
                  <div style={{ fontSize: 12, color: "#6b7280", width: 18, textAlign: "center" }}>{isOpen ? "▾" : "▸"}</div>

                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 950, fontSize: 13, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {g.part.part_id} — {g.part.name} <span style={{ fontSize: 12, color: "#9ca3af" }}>({g.rows.length})</span>
                    </div>

                    <div style={{ display: "flex", gap: 6, marginTop: 6, alignItems: "center" }}>
                      {thumbs.map((t, i) => (
                        <MiniThumb key={`${g.part.id}-t-${i}`} src={t as string} />
                      ))}
                      {Array.from({ length: showPlaceholders }).map((_, i) => (
                        <MiniThumb key={`${g.part.id}-p-${i}`} src={null} />
                      ))}
                      <div style={{ fontSize: 11, color: "#9ca3af" }}>{thumbs.length > 0 ? "preview" : "no images yet"}</div>
                    </div>
                  </div>

                  <div style={{ fontSize: 12, color: "#9ca3af", whiteSpace: "nowrap" }}>{isOpen ? "hide" : "show"}</div>
                </button>
                    ...
                    {g.rows.map((pc, pcIdx) => (
                      <button
                        ...
                        style={{
                          ...
                          gap: 8,
                          alignItems: "center",
                          borderTop: pcIdx === 0 ? "none" : "1px solid #edf2f7",
                        }}
                      >
                        ...
                        <div style={{ fontSize: 12, color: "#111827", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", minWidth: 0 }}>
                          {pc.color?.name ?? "—"}
